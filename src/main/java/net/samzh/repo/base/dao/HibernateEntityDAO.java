...
import net.samzh.repo.util.BeansUtils;
...
public class HibernateEntityDAO<T, PK extends Serializable> extends
		HibernateDaoSupport implements IEntityDAO<T, PK> {
	...
	public Page pagedQuery(Criteria criteria, int pageNo, int pageSize) {
		...
		try {
			orderEntries = (List) BeansUtils.forceGetProperty(impl,
					"orderEntries");
			BeansUtils.forceSetProperty(impl, "orderEntries", new ArrayList());
		} catch (Exception e) {
		...
		try {
			BeansUtils.forceSetProperty(impl, "orderEntries", orderEntries);
		} catch (Exception e) {
			throw new InternalError(" Runtime Exception impossibility throw ");
		}
		// 返回分页对象
	 ...
	 */
	@SuppressWarnings("unchecked")
	public PK getId(Class<T> entityClass, T entity)
